...
# Added: DISABLED=1 support: red text in dropdowns, red editor warning, toggle button
...
import parser  # config_parser.py
...
class StorytoolsGUI:
    def __init__(self, root):
        ...
        self.jobtypes = ['ct_flux_t2i', 'ct_wan2_5s', 'ct_qwen_i2i', 'ct_qwen_cameratransform', 'ct_ltx2_i2v']
    ...
    def create_widgets(self):
        ...
        self.toggle_disable_btn = ctk.CTkButton(
            self.main_frame,
            text="Disable Shot",
            width=140,
            fg_color="#D32F2F",
            hover_color="#B71C1C",
            command=self.toggle_shot_disabled
        )
        self.toggle_disable_btn.pack(pady=(4,8), padx=4)
    ...
    def open_seq_popup(self):
        ...
        for seq in self.sequences:
            has_disabled = any(
                any(
                    re.match(r'^\s*DISABLED\s*=\s*(1|yes|true|on)\s*$', line.strip(), re.IGNORECASE)
                    for line in self.original_lines[start:end]
                )
                for (s, sh), (start, end) in self.shot_ranges.items() if s == seq
            )
            text_color = "#FF5252" if has_disabled else "white"
            btn = ctk.CTkButton(
                scroll,
                text=seq + (" ⚠" if has_disabled else ""),
        ...
        for child in scroll.winfo_children():
            if isinstance(child, ctk.CTkButton) and self.selected_seq == child.cget("text").split(" ")[0]:
    ...
    def open_shot_popup(self):
        ...
        for shot_id in shots:
            ...
            key = (self.selected_seq, shot_id)
            is_disabled = False
            if key in self.shot_ranges:
                start, end = self.shot_ranges[key]
                block_lines = self.original_lines[start:end]
                is_disabled = any(
                    re.match(r'^\s*DISABLED\s*=\s*(1|yes|true|on)\s*$', line.strip(), re.IGNORECASE)
                    for line in block_lines
                )

            text_color = "#FF5252" if is_disabled else "white"
            suffix = " [DISABLED]" if is_disabled else ""
            btn = ctk.CTkButton(
                scroll,
                text=display_text + suffix,
                fg_color="transparent",
                hover_color="#3b3b3b",
                anchor="w",
                text_color=text_color,
        ...
        for child in scroll.winfo_children():
            if isinstance(child, ctk.CTkButton) and self.selected_shot == child.cget("text").split(" - ")[0]:
                child.configure(fg_color="#1f538d")
    ...
    def open_jobtype_popup(self):
        ...
        for sub_data in shot_container.values():
            jt_str = sub_data.get('JOBTYPE', '') or sub_data.get('IMAGE_JOBTYPE', '') or sub_data.get('VIDEO_JOBTYPE', '')
            if jt_str:
                jts = [jt.strip() for jt in jt_str.split(',') if jt.strip()]
                available_jts.update(j for j in jts if j in self.jobtypes)

        options = sorted(available_jts) if available_jts else ["None"]
        ...
        self.jobtype_popup.geometry(f"220x280+{x}+{y}")
        ...
        for opt in options:
            btn = ctk.CTkButton(
                ...
                hover_color="#3b3b3b",
                anchor="w",
                command=lambda o=opt: self.select_jobtype_and_close(o)
            )
            btn.pack(fill="x", pady=1)

            if self.selected_jobtype == opt:
    ...
    def select_jobtype_and_close(self, jobtype):
        self.selected_jobtype = jobtype if jobtype != "None" else None
        self.jobtype_var.set(jobtype)
        self.on_jobtype_change(jobtype)
        self.close_all_popups()
    ...
    def open_mode_popup(self):
        ...
        for mode in self.qwen_modes:
            ...
            if self.selected_qwen_mode == mode:
                btn.configure(fg_color="#1f538d")
    ...
    def select_mode_and_close(self, mode):
        ...
        self.close_all_popups()
        self.save_qwen_mode_to_config()

    def on_mode_change(self, *args):
        self.selected_qwen_mode = self.qwen_mode_var.get()
        self.mode_var.set(self.selected_qwen_mode)

    def save_qwen_mode_to_config(self):
        if not self.config_path or not self.original_lines:
            return

        found = False
        new_lines = []
        target = f"QWEN_CAMERATRANSFORMATION_MODE={self.selected_qwen_mode}"
        ...
        if not found and self.original_lines:
            new_lines.append(target + "\n")
    ...
    def toggle_shot_disabled(self):
        if not self.selected_seq or not self.selected_shot or not self.original_lines:
            messagebox.showwarning("No shot selected", "Select a shot first.")
            return

        key = (self.selected_seq, self.selected_shot)
        if key not in self.shot_ranges:
            messagebox.showerror("Error", "Shot block position not found.")
            return
        ...
        for i, line in enumerate(block_lines):
            stripped = line.strip()
            if re.match(r'^DISABLED\s*=\s*(1|yes|true|on)$', stripped, re.IGNORECASE):
                is_disabled = True
                disabled_line_idx = i
                break
        ...
        try:
            if is_disabled:
                new_block_lines = [line for i, line in enumerate(block_lines) if i != disabled_line_idx]
                button_text = "Disable Shot"
                button_fg = "#D32F2F"
                button_hover = "#B71C1C"
            else:
                new_block_lines = block_lines.copy()
                if new_block_lines and not new_block_lines[-1].strip():
                    new_block_lines.insert(-1, "DISABLED=1\n")
                else:
                    new_block_lines.append("DISABLED=1\n")
                button_text = "Enable Shot"
                button_fg = "#4CAF50"
                button_hover = "#388E3C"

            new_block_text = ''.join(new_block_lines)
            new_original = self.original_lines[:start] + [new_block_text] + self.original_lines[end:]

            with open(self.config_path, 'w', encoding='utf-8', newline='') as f:
                f.writelines(new_original)

            self.original_lines = new_original
            self.refresh_config()
            self.on_shot_change(self.selected_shot)

            self.toggle_disable_btn.configure(
                text=button_text,
                fg_color=button_fg,
            ...
            messagebox.showinfo("Success", f"Shot {self.selected_shot} {status}.")
        except Exception as e:
    ...
    def on_seq_change(self, selection):
        ...
        if not self.selected_seq:
            ...
            self.update_host_selection()
            self.update_mode_visibility()
            self.toggle_disable_btn.configure(state="disabled")
            return
    ...
    def on_shot_change(self, selection):
        ...
        if key in self.shot_ranges:
            ...
            if is_disabled:
                self.shot_editor.configure(text_color="#FF5252")
                self.editor_info.configure(
                    text="This shot is DISABLED — will be skipped during Run",
                    text_color="#FF5252"
                )
                self.toggle_disable_btn.configure(
                    text="Enable Shot",
                ...
                self.shot_editor.configure(text_color="white")
                self.editor_info.configure(
                    text="Drop liked Flux PNG here to set SEED_START • globals excluded",
                    text_color="gray"
                )
        ...
        self.update_mode_visibility()
        self.update_host_selection()
        self.toggle_disable_btn.configure(state="normal")
